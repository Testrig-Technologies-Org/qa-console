        ...
        <div className="w-full max-w-lg bg-card border border-border rounded-lg shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 max-h-[90vh] flex flex-col">
          <div className="px-6 py-4 border-b border-border bg-muted/5 flex justify-between items-center shrink-0">
            <div className="flex items-center gap-3">
              ...
              <span className="text-[11px] font-black text-foreground tracking-wide">Project Registered</span>
            </div>
      ...
      <div className="w-98 bg-card border border-border rounded-lg shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 border-b border-border bg-muted/5 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[11px] font-black text-foreground tracking-wide">Resource Registration Protocol</span>
          </div>
          <button onClick={handleClose} className="text-muted hover:text-foreground transition-colors">
            <X size={18} />
          </button>
        ...
        <form onSubmit={handleSubmit}>
          <div className="p-8 space-y-6">
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-muted tracking-wide block">Instance Name</label>
              <input 
                required
                autoFocus
                value={formData.name}
                onChange={(e) => setFormData({...formData, name: e.target.value})}
                className="w-full bg-background border border-border p-3 text-sm text-foreground outline-none focus:border-indigo-500 transition-all placeholder:text-muted/30"
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-muted tracking-wide block">Framework</label>
                <div className="relative">
                  ...
                  >
                    <option value="cypress">Cypress Engine</option>
                    <option value="playwright">Playwright Engine</option>
                  </select>
            ...
            <div className="bg-indigo-500/5 border border-indigo-500/20 p-4 border-l-2 border-l-indigo-500">
               <div className="flex gap-3">
                 ...
                 <p className="text-[9px] text-muted leading-relaxed">
                   Linking requires a generated <span className="text-indigo-500 font-bold underline">Project UUID</span>. Registering will create a unique handshake key for your CI pipeline logs.
                 </p>
               </div>
            </div>
          </div>
