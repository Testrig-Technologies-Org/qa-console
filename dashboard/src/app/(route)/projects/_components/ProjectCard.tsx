...
export function ProjectCard({ project }: { project: any }) {
  ...
  return (
    <Link
      ...
      className={cn(
        "group relative bg-card border border-border border-t-2 p-6 flex flex-col justify-between min-h-[230px] transition-all hover:bg-muted/5 shadow-sm rounded-lg duration-300",
      ...
      <div>
        <div className="flex justify-between items-start mb-6">
          ...
          <div className="text-right">
             <p className="text-[8px] font-black text-muted tracking-wide mb-1">Coverage</p>
             <p className="text-xl font-bold text-foreground tabular-nums">{project.coverage || '0%'}</p>
          </div>
        </div>

        <h3 className="text-sm font-black text-foreground tracking-tight group-hover:text-indigo-500 transition-colors">
          {project.name}
        </h3>
        
        <div className="flex items-center gap-3 mt-2">
          <span className="text-[9px] font-bold text-muted tracking-wide">{project.type}</span>
          <span className="w-1 h-1 bg-border rounded-full" />
          <span className="text-[9px] font-bold text-muted tracking-wide opacity-70">Node Stable</span>
        </div>
      </div>

      <div className="mt-8 pt-4 border-t border-border/50 flex items-center justify-between">
        <div className="flex flex-col">
           <span className="text-[8px] text-muted font-black tracking-tighter leading-none mb-1">History Objects</span>
