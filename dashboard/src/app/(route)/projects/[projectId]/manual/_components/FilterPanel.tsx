...
export function FilterPanel({ idFilter, setIdFilter, titleFilter, setTitleFilter, moduleFilter, setModuleFilter, masterCases }: any) {
  return (
    <div className="bg-card border border-border p-4 rounded-sm grid grid-cols-1 md:grid-cols-3 gap-4 shadow-sm">
      ...
      <div className="flex items-center px-4 gap-3 bg-background border border-border rounded-sm group focus-within:border-indigo-500">
        ...
        <select className="bg-transparent border-none focus:ring-0 w-full py-3 text-[10px] font-black text-muted tracking-wide cursor-pointer" value={moduleFilter} onChange={e => setModuleFilter(e.target.value)}>
