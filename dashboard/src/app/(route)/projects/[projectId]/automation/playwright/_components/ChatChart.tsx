...
function ChartHeader({ result, pinButton }: { result: any; pinButton?: React.ReactNode }) {
  ...
  return (
    <div className="px-1 mb-1.5 flex items-center justify-between gap-2">
      <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[9px] font-bold text-muted uppercase tracking-widest">
...
export function ChatChart({ name, args, result, projectId, onPinned }: ChatChartProps) {
  ...
  const pinButton = projectId ? (
    <button
      ...
      className={cn(
        "shrink-0 flex items-center gap-1 px-2 py-1 text-[9px] font-bold uppercase tracking-widest border rounded-full transition-colors",
        pinState === 'pinned'
