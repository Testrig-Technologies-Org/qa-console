...
export function BuildIntelligencePanel({ buildId, buildData }: BuildIntelligencePanelProps) {
    ...
    return (
        <div className="bg-card border border-border rounded-lg overflow-hidden shadow-sm font-mono mb-6 transition-colors duration-300">
            {/* Toolbar */}
            <div className="px-6 py-3 border-b border-border bg-muted/5 flex justify-between items-center">
                ...
                <div className="flex items-center gap-3">
                    {(loading || streaming) && (
                        <button onClick={cancelStream} className="text-[9px] font-bold text-rose-500 hover:text-rose-400 flex items-center gap-2">
                            <X size={10} /> Cancel
                        </button>
                    )}
                    {report && !loading && !streaming && (
                        <button onClick={generateReport} className="text-[9px] font-bold text-muted hover:text-foreground flex items-center gap-2 transition-colors">
            ...
            <div className="p-6 min-h-[180px] flex flex-col justify-center">
                ...
                {loading && !streaming && (
                    <div className="flex flex-col items-center justify-center space-y-4 py-8">
                        ...
                        <p className="text-[9px] font-black tracking-wide animate-pulse text-muted">
                    ...
                    <div className="space-y-4 animate-in fade-in duration-300">
                        <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-[9px] font-black text-emerald-500 tracking-wide">Streaming Trace</span>
                            </div>
                            <div className="text-[9px] text-muted font-bold">{tokensReceived} Tokens</div>
                        </div>
                ...
                {error && !loading && !streaming && (
                    <div className="flex flex-col items-center justify-center text-rose-500 gap-3 border border-rose-500/20 bg-rose-500/5 p-8">
                        <AlertCircle size={24} />
                        <span className="text-[10px] font-bold tracking-wide text-center">{error}</span>
                        <button onClick={generateReport} className="px-4 py-2 bg-rose-500/10 border border-rose-500/20 text-[10px] font-bold hover:bg-rose-500/20 transition-all">
                            Retry Analysis
                        </button>
                    </div>
                )}
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-in fade-in duration-500">
                        <div className="lg:col-span-5 space-y-6">
                            <section className="space-y-2">
                                <h4 className="text-[9px] font-bold text-muted tracking-wide flex items-center gap-2">
                        ...
                        <div className="lg:col-span-7 space-y-6">
                            ...
                            <div className="bg-emerald-500/5 p-4 border border-emerald-500/20 border-l-2 border-l-emerald-500 flex items-start gap-4">
                                ...
                                <div>
                                    <p className="text-[10px] font-black text-emerald-600 dark:text-emerald-500 mb-1">Recommended Fix</p>
                ...
                {!loading && !streaming && !report && !error && (
                    <div className="flex flex-col items-center justify-center py-12 space-y-6 animate-in fade-in zoom-in-95 duration-500">
                        ...
                        <div className="text-center space-y-1">
                            <p className="text-[11px] font-black tracking-wide text-foreground">Heuristic Build Analysis</p>
                            <p className="text-[10px] text-muted font-bold tracking-tight">Generate AI insights for regression failures</p>
                        </div>
                        <button onClick={generateReport} className="flex items-center gap-3 px-8 py-3 bg-indigo-600 hover:opacity-90 text-white border border-indigo-400/50 shadow-xl transition-all group">
                            ...
                            <span className="text-[10px] font-black tracking-wide">Execute Intelligence Pipeline</span>
                        </button>
                        <p className="text-[9px] text-muted tracking-wider">Powered by Local LLM • {CONFIG.MODEL_NAME}</p>
                    </div>
                )}
            </div>
        </div>
