...
export function DashboardHeader({ 
  ...
  return (
    <header className="flex flex-col gap-1 font-mono transition-colors duration-300">
      ...
      <div className="flex flex-col lg:flex-row bg-card border border-border rounded-lg shadow-xl">
        ...
        <div className="flex-1 p-6 border-b lg:border-b-0 lg:border-r border-border flex items-center gap-6">
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              <span className="text-[9px] bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-500/20 px-2 py-0.5 font-bold tracking-wide">
        ...
        <div className="w-full lg:w-96 p-6 bg-muted/5 flex flex-col justify-center gap-3">
          <div className="flex justify-between items-end">
             <span className="text-[10px] font-black text-muted tracking-wide">Automation Coverage</span>
             <span className="text-xs font-bold text-foreground tabular-nums">{coveragePercent}%</span>
          </div>
          {/* Square Progress Bar */}
          <div className="h-2 w-full bg-border rounded-lg overflow-hidden">
            <div 
              className="h-full bg-indigo-500 transition-all duration-1000 shadow-[0_0_10px_rgba(99,102,241,0.3)]" 
              style={{ width: `${coveragePercent}%` }} 
            />
          </div>
          <div className="flex justify-between items-center text-[9px] text-muted font-bold">
...
export function FilterButton({ active, label, count, onClick, color }: any) {
  ...
  return (
    <button 
      onClick={onClick} 
      className={cn(
        "flex-1 flex items-center justify-between px-4 py-2 border transition-all rounded-lg",
        colorClasses[color]
      )}
    >
