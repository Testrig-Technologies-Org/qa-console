...
export async function createProject(formData: {
  ...
  try {
    ...
    // Re-fetch to get the apiKey generated by the schema's $defaultFn on insert
    const created = await db.query.projects.findFirst({ where: eq(projects.id, insertedId) });

    // Revalidate paths
    revalidatePath('/projects');
    revalidatePath('/dashboard');

    return { success: true, id: insertedId, apiKey: created?.apiKey ?? null };
  } catch (error: any) {
...
export async function regenerateProjectApiKey(projectId: number) {
  try {
    const { userId } = await auth();
    if (!userId) {
      return { success: false, error: 'Unauthorized' };
    ...
    const newKey = generateProjectApiKey();
